...
const PendingSellers: React.FC<Props> = (props) => {
  ...
  useEffect(() => {
    const fetchAllPendingSeller = async () => {
      if (user_slug) {
        const { res, err } = await EcommerceApi.getAllSeller(
          `sortBy=${sortBy}&sortType=${sortType}&search=${searchString}&status=inactive`
        );
        if (err) {
          console.log(err);
        } else {
  ...
  const handleDelete = async () => {
    ...
    if (res) {
      setDeleteModalSlug("");
      const remainingCustomers = sellersData.filter(
  ...
  return (
    <div className="w-full">
      <DashboardBreadcrumb
        headline="Pending Seller List"
        slug="Pending Seller List"
